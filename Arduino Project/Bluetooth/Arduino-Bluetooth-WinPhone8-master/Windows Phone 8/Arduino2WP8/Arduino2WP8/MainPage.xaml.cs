...
namespace HC_Bluetooth
{
    public partial class MainPage : PhoneApplicationPage
    {
        ...
        StreamSocket bluetoothSoket; // Socket used to Communicate with the Arduino/BT Module
        ...
        */
        ...
        string bluetootnStatus = ""; // Used to Store if we can send Message (e.g. yes or no)
        ...
        {
            if (bluetoothSoket == null) // If we don't have a connection, Send Error Control
            {
               // MessageBox.Show("Please connect to a device first."); // Alert the user with a Notification (Optional)
                ...
                if (bluetoothSoket != null) // Since we have a Connection
                {
                    var datab = GetBufferFromByteArray(UTF8Encoding.UTF8.GetBytes(WhatToSend)); // Create Buffer/Packet for Sending
                    await bluetoothSoket.OutputStream.WriteAsync(datab); // Send our Message to Connected Arduino
        ...
        private async void lstBTPaired_Tap_1(object sender, GestureEventArgs e)
        {
            if (lstBTPaired.SelectedItem == null) // To prevent errors, make sure something is Selected
            {
                ...
                if (lstBTPaired.SelectedItem != null) // Just making sure something was Selected
                {
                    ...
                    bluetoothSoket = new StreamSocket(); // Create a new Socket Connection
                    await bluetoothSoket.ConnectAsync(PF[lstBTPaired.SelectedIndex].HostName, "1"); // Connect using Socket to Selected Item
                    ...
                    await bluetoothSoket.OutputStream.WriteAsync(datab); // Send Arduino Buffer/Packet Message
        ...
        void PeerFinder_TriggeredConnectionStateChanged(object sender, TriggeredConnectionStateChangedEventArgs args)
        {
            ...
            if (args.State == TriggeredConnectState.Failed)
            {
                ...
                bluetootnStatus = "no"; // Not connected
                return;
            }
            ...
            if (args.State == TriggeredConnectState.Completed)
            {
                ...
                bluetootnStatus = "yes"; // Means we are connected
                MessageBox.Show("hi");
            }
        }
